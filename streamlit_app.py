...
def limpar_dados(dados_fundos_total):
    ...
    colunas_para_excluir = ['Quantidade Venda Negociada', 'Quantidade Aquisição Negociada',
                            ...
                            'Descrição Indexador Pós-Fixado','Percentual Indexador Pós-Fixado', 'Percentual Cupom Pós-Fixado', 
                            'Percentual Taxa Pré-Fixada',  'Agência de Risco',  'Data Risco',  'Grau de Risco', 
                            'Título Cetip', 'Título Garantia',  'CNPJ Instituição Financeira Coobrigada',  
                            'Investimento Coletivo', 'Gestor Investimento Coletivo', 'Código Ativo BV Mercado', 
                            'Descrição Ativo Exterior', 'Quantidade Ativo Exterior', 'Valor Ativo Exterior']
...
# Título do Dashboard com um tema mais atraente
st.markdown("<h1 style='text-align: center; color: #007bff;'>Fundos de Investimento - CVM</h1>", unsafe_allow_html=True)

# Dividindo a tela em duas colunas para melhorar o layout
...
with col1:
    ...
    tipo_aplicacao_filtro = st.sidebar.multiselect("Filtrar por Tipo de Aplicação:", options=dados_fundos_total['Tipo Aplicação'].unique())
    ...
    if cnpj_filtro and not dados_filtrados.empty:
        ...
        st.markdown("<h3>Informações do Fundo:</h3>", unsafe_allow_html=True)
...
with col2:
    st.markdown("<h3>Gráfico de Distribuição por Tipo de Aplicação:</h3>", unsafe_allow_html=True)
